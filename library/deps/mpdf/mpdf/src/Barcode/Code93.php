 ...
 */
class Code93 extends \Academy\Mpdf\Barcode\AbstractBarcode implements \Academy\Mpdf\Barcode\BarcodeInterface
{
	...
	private function init($code)
	{
		...
		for ($i = 0; $i < $clen; ++$i) {
			if (ord($code[$i]) > 127) {
				throw new \Academy\Mpdf\Barcode\BarcodeException(sprintf('Invalid character "%s" in CODE93 barcode value "%s"', $code[$i], $code));
			}
		...
		for ($i = 0; $i < $clen; ++$i) {
			...
			if (!isset($chr[$char])) {
				// invalid character
				throw new \Academy\Mpdf\Barcode\BarcodeException(sprintf('Invalid CODE93 barcode value "%s"', $code));
			}
			for ($j = 0; $j < 6; ++$j) {
				if (($j % 2) == 0) {
					$t = true; // bar
