...
namespace Academy\Mpdf\Image;

use Academy\Mpdf\AssetFetcher;
...
use Academy\Mpdf\Gif\Gif;
use Academy\Mpdf\Language\LanguageToFontInterface;
use Academy\Mpdf\Language\ScriptToLanguageInterface;
...
class ImageProcessor implements \Academy\Psr\Log\LoggerAwareInterface
{
	...
	/**
	 * @var \Academy\Mpdf\Image\ImageTypeGuesser
	 */
	private $guesser;
	...
	public function getImage(&$file, $firstTime = true, $allowvector = true, $orig_srcpath = false, $interpolation = false)
	{
		...
		if (!$data) {
			...
			} catch (\Academy\Mpdf\Exception\AssetFetchingException $e) {
	...
	private function gzCompress($data)
	{
		if (!function_exists('gzcompress')) {
			throw new \Academy\Mpdf\MpdfException('gzcompress is not available. install ext-zlib extension.');
		}
	...
	private function imageError($file, $firstTime, $msg)
	{
		...
		if ($firstTime && ($this->mpdf->showImageErrors || $this->mpdf->debug)) {
			throw new \Academy\Mpdf\MpdfImageException(sprintf('%s (%s)', $msg, substr($file, 0, 256)));
		}
	...
	public function processJpg($data, $file, $firstTime, $interpolation)
	{
		...
		if ($a[2] === 'DeviceCMYK' && ($this->mpdf->restrictColorSpace === 2 || ($this->mpdf->PDFA && $this->mpdf->restrictColorSpace !== 3))) {
			...
			if (!function_exists('gd_info')) {
				throw new \Academy\Mpdf\MpdfException(sprintf('JPG image may not use CMYK color space (%s).', $file));
			}
	...
	public function processPng($data, $file, $firstTime, $interpolation)
	{
		...
		if ($firstTime && ($colspace === 'DeviceRGB' || $colspace === 'Indexed') && ($this->mpdf->PDFX || $this->mpdf->restrictColorSpace === 3)) {
			...
			if ($pngalpha) {
				if ($this->mpdf->PDFA) {
					throw new \Academy\Mpdf\MpdfException(sprintf('PDFA1-b does not permit images with alpha channel transparency (%s).', $file));
				}
